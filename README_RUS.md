...
Вероятно, может возникнуть такая ситуация, когда результат работы скрипта необходимо сохранить. В этом случае для
выполняемого кода не "как часть pipeline'а" доступны два способа: записать результат выполнения действий скрипта в файл
(с последующей [сборкой артефактов](#action-сборка-файлов-артефактов)), или сохранение результата в переменную 
окружения. Для кода, выполняемого "как часть pipeline'а" помимо перечисленных способов доступно так же создание ключа
во встроенной в pipeline переменной `universalPipelineWrapperBuiltIns` (см.
[Встроенные в pipeline переменные](#встроенные-в-pipeline-переменные)). Значения переменных окружения, как и встроенная
