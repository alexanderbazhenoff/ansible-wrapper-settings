...
# (пустое значение параметра) завершит выполннение pipeline ошибкой. Если параметр `PASSWORD` не указан, то в консоли
...
# "перевод строки" для подстановки внутри ansible inventory (не включен в пример, но подразумевается). Обратите так же
# внимание на синтаксис в значении поля 'to' в ключе regex_replace параметра `IP_ADDRESSES`. Если стадии pipeline будут
# использовать эту переменную, то ее значение будет так же иметь отформатированный вид: IP (или хосты) будут разделены
# переводом строк, а не пробелом.
