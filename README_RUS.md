...
в соответствующий словарь [`optional`](#optional) за-счет указания дополнительных опций ключа [`on_empty`](#required)
(см. [Пример 2](#пример-2)). Pipeline может так же иметь только необязательные параметры (см. [Пример 4](#пример-4)), 
или вовсе их не иметь ни одного параметра (в таком случае указывается только пустой ключ `parameters`).
