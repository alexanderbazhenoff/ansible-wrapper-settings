...
каждый элемент списка является параметром и имеет следующие ключи (см. [Пример 15](#пример-15)):
    ...
    - **type** `[строка]` *(обязательный)* - тип параметра: `string`, `text`, `password`, `choice`, или `boolean`.
    - **parameter** `[строка или логический]` *(обязательный)* - строчное или boolean значение параметра в зависимости 
      от типа параметра.
