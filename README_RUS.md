...
вовсе их не иметь ни одного параметра (в таком случае указывается только пустой ключ `parameters`).
...
Ключ содержит скрипты, где каждый вложенный ключ является именем этого скрипта, а его значения - параметрами скрипта и
содержимым скрипта. Скрипты так же можно выполнять "как часть pipeline'а" (см.
[Пример 18](#пример-18)).

- **pipeline** `[boolean]` *(необязательный)* - если указан, то скрипт выполняется "как часть pipeline" (см.
[Пример 18](#пример-18)) и тогда обязательно указание, в каком инструменте для CI (или окружении) должен этот скрипт
  ...
  окружения) (см. [Пример 18](#пример-12)).
...
находиться соответствующий runtime-окружению код.

#### Пример 18

```yaml
# Фрагмент конфигурационного файла с описанием действия по запуску кода "как часть pipeline" и заданием кода для
# Jenkins и Teamcity.
actions:
...
который будет использоваться для всех этих playbook'ов (см. [Пример 19](#пример-19)). Для каждого playbook'а может быть
так же задан свой inventory: в этом случае inventory создаются с тем же именем, что и playbook, которому он 
соответствует (см. [Пример 20](#пример-20)).
...
каждом действии и стадии pipeline. Содержимое этой идентично *multilineReport*, только имеет удобный для чтения формат.
В отличие от вывода такой же таблицы в console Jenkins, или Teamcity эта переменная не содержит цветовых кодов (ASCII
colours) и, таким образом, полезна при отправке различных уведомлений.
...
[example-pipeline.yml](settings/example-pipeline.yaml), который содержит максимально поддерживаемый набор опций
Universal wrapper pipeline с комментариями).
